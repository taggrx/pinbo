...
export const DEPLOYER_ADDRESS = '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266' as const;
...
export default async function globalSetup() {
	...
	if (!addrMatch) {
		throw new Error(`Could not parse contract address from forge output:\n${deployOutput}`);
	}
	...
	// Write full config for the frontend server and individual test fixtures.
	const config = {
		anvilPid: anvil.pid,
		contractAddress,
		deployBlock,
	...
	// Start the frontend dev server now that the config is ready.
	console.log('[e2e:setup] Starting frontend...');
	const frontend = spawn('node', [join(ROOT, 'e2e', 'start-frontend.mjs')], {
		stdio: ['ignore', 'pipe', 'pipe'],
		detached: false,
		env: shellEnv,
	});
	frontend.on('error', (e) => {
		throw new Error(`Frontend failed to start: ${e.message}`);
	});
	...
	if (Date.now() >= frontendDeadline) throw new Error('Frontend did not become ready in time');
	console.log('[e2e:setup] Frontend ready.');

	// Persist the frontend PID so globalTeardown can stop it.
	writeFileSync(CONFIG_PATH, JSON.stringify({ ...config, frontendPid: frontend.pid }, null, 2));
