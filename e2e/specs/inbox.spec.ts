...
test.describe('Inbox (ADDRESS topic / DMs)', () => {
	...
	test('sent DM encodes ADDRESS topic with recipient bytes on-chain', async ({
		...
		await page.locator('.toastui-editor-md-container textarea').fill('Topic encoding test DM');
		await page.getByRole('button', { name: 'SEND' }).click();
		...
		expect(bytesToHex(addrTopic![1]).toLowerCase()).toBe(testConfig.bobAddress.toLowerCase());
	});
	...
	test('multiple DMs from different senders all appear in inbox', async ({ alicePage: page }) => {
		const dmTopics: Array<[number, Uint8Array]> = [
			[TOPIC_TYPE.ADDRESS, hexToBytes(testConfig.aliceAddress)],
		];
