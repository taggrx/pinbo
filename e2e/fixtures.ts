...
// ---------------------------------------------------------------------------
export const testConfig = JSON.parse(readFileSync(CONFIG_PATH, 'utf8')) as {
	anvilPid: number;
...
function buildMockProviderScript(account: string, chainIdHex: string, rpcUrl: string): string {
	const template = readFileSync(join(process.cwd(), 'e2e', 'mock-provider.js'), 'utf8');
	return template
...
export const aliceWallet: TestWallet = {
	...
	providerScript: buildMockProviderScript(testConfig.aliceAddress, chainIdHex, testConfig.rpcUrl),
};

export const bobWallet: TestWallet = {
	address: testConfig.bobAddress,
	privateKey: testConfig.bobKey,
	providerScript: buildMockProviderScript(testConfig.bobAddress, chainIdHex, testConfig.rpcUrl),
};
