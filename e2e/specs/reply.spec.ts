...
test.describe('Replies (REPOST topic)', () => {
	test('REPLY button appears on permalink when connected', async ({ alicePage: page }) => {
		...
		await expect(page.locator('.message.card').getByRole('button', { name: 'REPLY' })).toBeVisible({
			timeout: 10_000,
		});
	});
	...
	test('clicking REPLY shows the reply form with reply placeholder', async ({
		...
		await expect(
			page.locator('.post-section').getByRole('button', { name: 'REPLY' })
		).toBeVisible();
	});
	...
	test('reply encodes REPOST topic with parent tx hash on-chain', async ({ alicePage: page }) => {
	...
	test('repost card is rendered inside the parent message', async ({ alicePage: page }) => {
		...
		const replyTopics: Array<[number, Uint8Array]> = [[TOPIC_TYPE.REPOST, hexToBytes(parentHash)]];
